...
<<mapping_sync#using-launchpad-pro,Launchpad Pro>> instructions.
...
so they do not attempt to mimic the cue grid colors. Instead, colors
are used to represent cue state.

=== Launchpad Mini and S Cue Grid Colors

Pads which have no cues associated with them are dark. If the pad has
a cue that is not running, it will be amber. If the cue conflicts with
a cue that is running, it will be a dim amber. Cues which are running
are bright green; cues which are ending blink red. Cues which run only
while you hold them down are bright amber rather than green while you
hold them, so you can tell they will end when you let them go. As with
other controllers, if you hold down the Shift button while launching
such a momentary cue, it will stay running even after you let it go,
and in that case will be green.
