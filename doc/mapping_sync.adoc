...
In order to do that, download the Afterglow Traktor Controller
Mapping,
https://raw.githubusercontent.com/brunchboy/afterglow/master/doc/assets/Afterglow.tsi[Afterglow.tsi],
...
as a Control Change message on controller `0`. (The same number to deck
correspondence is used.) When no deck is Tempo Master, a value Control
Change with value `0` is sent on controller `0`.

Afterglow will only look for the special Control Change messages if
the name of the MIDI device on which the messages are being received
contains the string "Traktor". This will automatically be the case
with Internal Sync on the Mac; if you are using MIDI over a network,
or creating your own virtual MIDI devices on Windows, be sure to make
Traktor part of the MIDI port name, so Afterglow can know to watch for
...
also adjust the BPM by turning the BPM encoder, which is the leftmost
...
number `1`. You can adjust those with the beat encoder, the rightmost
encoder below the metronome button.
