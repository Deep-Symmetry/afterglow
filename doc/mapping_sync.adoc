...
to right, with the most recent effect on the right.

TODO: Photo of effect area, link to discussion of adjusting effects.
...
Cues can also be configured to react to pressure sensitivity, so that
as you vary the pressure with which you are holding down the cue pad,
some visible parameter of the cue is altered. The strobe and sparkle
cues in created by
...
active arrow scrolls the view one &ldqou;page&rdquo; in that
direction. In this photo below, it is currently possible to scroll up
and to the right.

image::assets/PushScroll.jpg[Push scroll diamond]

If you hold down the `Shift` button, the arrows gain a different
purpose, allowing you to scroll the text area left and right, to see
and <<mapping_sync#effect-control,adjust>> all of the currently
