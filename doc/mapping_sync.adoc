...
NOTE: Currently only the original Push hardware is supported. Ableton
has *just* released detailed documentation of the Push 2 interface,
and I am currently working on support for it. I have already
https://github.com/brunchboy/wayang#wayang[developed] a way to draw on
the graphical display, and am working on using that to draw Afterglow
effect and parameter information, and adjusting to the other
differences in the way the Push 2 behaves. I can already tell that the
results are going to be delightful.
...
cue was defined with adjustable variables for its effect, they will
...
current default show like this, as long as its Standalone Port shows
up in your MIDI environment with the standard name `"Standalone
Port"`:

[source,clojure]
----
(require '[afterglow.controllers :as ct])
(def pro-watcher (ct/auto-bind *show* "Standalone Port"))
----
...
Assuming you have a Launchpad Mk2 connected to the machine running
Afterglow, you can activate its mapping and attach it to the current
default show like this, as long as its ports show up in your MIDI
environment with the standard names which include the string `"Mk2"`:

[source,clojure]
----
(require '[afterglow.controllers :as ct])
(def mk2-watcher (ct/auto-bind *show* "Mk2"))
----
...
current default show like this, as long as its ports show up in your
MIDI environment with the standard names of `"Launchpad Mini"` or
`"Launchpad S"`:

[source,clojure]
----
(require '[afterglow.controllers :as ct])
(def mini-watcher (ct/auto-bind *show* "Launchpad Mini"))
(def s-watcher (ct/auto-bind *show* "Launchpad S"))
----

You will see a brief startup animation, and Afterglow's Launchpad Mini
interface will start. (This also arranges for the Launchpad to be
re-bound to the show if it gets disconnected or powered off and then
reappears, which is very handy in a performance setting.) If you
wonder what would happen if you plugged in both a Launchpad Mini and
Launchpad S after running the above lines, the answer is that they
would both become active and you could use them at the same time. You
can have as many controller watchers and bindings running at the same
time as you like, as long as they can each properly identify the MIDI
ports to work with.
