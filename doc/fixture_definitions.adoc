...
top-level keys in the map returned, `:name`, and `:channels`. Those
...
|If present, identifies the fixture mode in which this definition map
 ...
 have different "personalities" which use a different number of DMX
 channels and provide a different set of features. Their feature
 definition functions will use a mode argument to determine the mode
 in which the fixture is operating, and return an appropriate map.
 That map will include the chosen mode keyword as the value at this
 key.
...
|If this fixture is a moving head capable of pan movements, this entry
 ...
 <<show_space#show-space,Show Space>>.
