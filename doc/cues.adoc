...
|If present, with a true value, requests that the function value be
 adjusted by strike velocity and aftertouch pressure while the
 operator is holding down the button or pad which launched the cue, on
...
|If present (and `:velocity` is active), specifies the smallest value
 the function should be set to by MIDI strike velocity and aftertouch
