...
at a time in an 8&times;8 grid, and this view can be scrolled around
within the larger grid. The user can configure ("`link`")
the web interface to track the scroll position of a registered grid
controller. When that is done, scrolling either interface will cause
the other to scroll in the same way, so the browser window can act as
documentation to help the user learn the cue names associated with
each available cue pad on the controller.
...
|Sets the color of the cue within the grid for hinting about purpose
and relatedness to help operators learn and understand the interface.
If not specified, white is used.

|===

TODO: Document `function-cue` and `compound-cues-effect`
