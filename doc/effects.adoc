...
Following example will assign red color on odd fixtures and blue color on
even fixtures. You need to use this code inside cue or use `end-effect!`.

[source,clojure]
----
(afterglow.effects/scene "Different colors"
  (show/add-effect! :color-odd-fixtures (afterglow.effects.color/color-effect
    "Plain red" (create-color "red") (show/fixtures-named "odd")))
  (show/add-effect! :color-even-fixtures (afterglow.effects.color/color-effect
    "Plain Blue" (create-color "blue") (show/fixtures-named "even")))
  )
----

To achieve the same effect inside a cue:
