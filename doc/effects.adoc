...
the purpose of the `scene` function in the `afterglow.effects`
namespace. It takes a name for the scene to be created, followed by
one or more effects to be grouped, and returns an effect which
combines them all under that name:
...
The The http://deepsymmetry.org/afterglow/doc/afterglow.effects.html#var-conditional-effect[conditional-effect] function in the `afterglow.effects` namespace
