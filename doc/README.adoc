...
terms of time (hours, minutes, seconds, and hundredths), and the show
Metronome (phrase, bar, beat, and hundredths).
...
If an effect was launched by a cue with any cue variables, they
appear next to the kbd:[End] button. Numeric cue variables can be
adjusted by dragging the associated slider. The current value appears
as a popup above the slider thumb when the mouse is over the slider.
Color cue variables can also be adjusted. The current color value
appears as a swatch; clicking on that swatch opens a color picker
interface which can be used to adjust the color parameter:
...
faders to do trigger cues and adjust variables used by the cues.
